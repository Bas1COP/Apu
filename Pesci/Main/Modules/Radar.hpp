...
void DrawRadarPointMiniMap(Vector3D EneamyPos, Vector3D LocalPos, float targetY, float eneamyDist, int TeamID, int xAxis, int yAxis, int width, int height, ImColor color, float targetyaw)
{
	bool out = false;
	Vector3D siz;
	siz.x = width;
	...
	pos.y = yAxis;
	bool ck = false;
	Vector3D single = RotatePoint(EneamyPos, LocalPos, pos.x, pos.y, siz.x, siz.y, targetY, 0.3f, &ck);
	if (eneamyDist >= 0.f && eneamyDist < Modules::Radar::miniMapRange)
	{
		for (int i = 1; i <= 30; i++)
		{
